    ...
    # ---------------------------------------------------------------------------
    # _start — entry point, calls main and exits with its return value
    # ---------------------------------------------------------------------------
    .globl _start
_start:
    call main
    ...
    # ---------------------------------------------------------------------------
    # __cppc_alloc — allocate memory via mmap syscall (no libc dependency)
    # rdi: size in bytes
    # returns: pointer in rax
    # ---------------------------------------------------------------------------
    .globl __cppc_alloc
__cppc_alloc:
    movq %rdi, %rsi         # arg2: length = size
    xorq %rdi, %rdi         # arg1: addr = NULL
    movq $3, %rdx           # arg3: prot = PROT_READ|PROT_WRITE
    movq $0x22, %r10        # arg4: flags = MAP_PRIVATE|MAP_ANONYMOUS
    movq $-1, %r8           # arg5: fd = -1
    xorq %r9, %r9           # arg6: offset = 0
    movq $9, %rax           # syscall 9 = mmap
    syscall
    ret

    # ---------------------------------------------------------------------------
    # __cppc_free — no-op (each allocation is a separate mmap)
    # ---------------------------------------------------------------------------
    .globl __cppc_free
__cppc_free:
    ret
...
__cxa_pure_virtual:
    movq $1, %rdi           # stderr fd
    leaq .Lpure_msg(%rip), %rsi
    movq $30, %rdx          # message length
